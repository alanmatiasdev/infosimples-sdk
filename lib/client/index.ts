...
const looper = (resource: any, auth: IConnectOpts) => {
  return Object.entries<any>(resource).reduce(
    (result: any, [method, func]: [method: string, func: any]) => {
      result[method] = isFunction(func)
        ? binder(func, auth)
        : looper(func, auth);
      return result;
    },
...
const connect = (opts: IConnectOpts): typeof resources => {
  ...
  const parsedResources = Object.entries<any>(resources).reduce(
    (result: any, [resource, methods]: [resource: string, methods: any]) => {
